...
void render(void) {
	...
	glUniformMatrix4fv(1, 1, GL_FALSE, lookat(vec3(0.0f, 0.0f, 5.0f), vec3(0.0f, 0.0f, 0.0f), vec3(0.0f, 1.0f, 0.0f)));
	for(int i = 0; i < len(cubePositions); i++) {
		mat4 model = translate(cubePositions[i]) * rotate(i * 20.0f, normalize(vec3(1.0f, 0.3f, 0.5f)));
		if(i % 3 == 0) {
			model *= rotate(degrees((float)getTime()), normalize(vec3(1.0f, 1.0f, 1.0f)));
		}
