...
mat4 myLookAt(GLCamera camera) {
    vec3 f = normalize(camera.front);
    vec3 u = normalize(camera.up);
    vec3 s = cross(f, u);
    u = cross(s, f);
    return (
	mat4(
	...
	mat4(
		...
		vec4(-camera.position[0], -camera.position[1], -camera.position[2], 1.0f)
	));
}
...
void render() {
	...
	glUniformMatrix4fv(1, 1, GL_FALSE, myLookAt(glcamera));
