...
glprogram_dl colorProgram;

void createProgram() {
	glshader_dl vertexShader;
	glshader_dl fragmentLightShader;
	glshader_dl fragmentColorShader;
	...
	cout<<glshaderCreate(&fragmentColorShader, GL_FRAGMENT_SHADER, "shaders/color.frag");

	cout<<glprogramCreate(&lightProgram, "Light", vector<glshader_dl>({vertexShader, fragmentLightShader}));
	cout<<glprogramCreate(&colorProgram, "Color", vector<glshader_dl>({vertexShader, fragmentColorShader}));
}
...
void render(void) {
	...
	glUseProgram(colorProgram.programObject);
	...
	glUniform3fv(4, 1, vec3(1.0f, 0.5f, 0.31f));
