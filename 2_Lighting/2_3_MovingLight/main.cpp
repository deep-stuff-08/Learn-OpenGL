...
vec3 lightPos = vec3(0.6f, 0.9f, 4.0f);
...
void createProgram() {
	...
	cout<<glshaderCreate(&vertexDiffuseShader, GL_VERTEX_SHADER, "shaders/phong.vert");
	cout<<glshaderCreate(&fragmentDiffuseShader, GL_FRAGMENT_SHADER, "shaders/phong.frag");
...
void render(void) {
	...
	lightPos[0] = 1.0f + sin(getTime()) * 2.0f;
	lightPos[1] = sin(getTime() / 2.0f) * 1.0f;
	...
	glUniform3fv(3, 1, lightColor);
	glUniform3fv(4, 1, objectColor);
	glUniform3fv(5, 1, lightPos);
	glUniform3fv(6, 1, glcamera.position);
