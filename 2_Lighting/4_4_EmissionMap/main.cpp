...
void createProgram() {
	...
	cout<<glshaderCreate(&vertexDiffuseShader, GL_VERTEX_SHADER, "shaders/emission.vert");
	cout<<glshaderCreate(&fragmentDiffuseShader, GL_FRAGMENT_SHADER, "shaders/emission.frag");

	cout<<glprogramCreate(&diffuseProgram, "Emission", vector<glshader_dl>({vertexDiffuseShader, fragmentDiffuseShader}));
}
...
void init(void) {
	...
	glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(vec3) + sizeof(vec3) + sizeof(vec2), (void*)(sizeof(vec3) + sizeof(vec3)));

	cout<<createTexture2D(&glo.tex[texDiffuseWood], "../../resources/container2.png");
	cout<<createTexture2D(&glo.tex[texSpecularWood], "../../resources/container2_specular.png");
	cout<<createTexture2D(&glo.tex[texEmissionMatrix], "../../resources/matrix.jpg");
...
void render(void) {
	...
	glBindTexture(GL_TEXTURE_2D, glo.tex[texDiffuseWood]);
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, glo.tex[texSpecularWood]);
	glActiveTexture(GL_TEXTURE2);
	glBindTexture(GL_TEXTURE_2D, glo.tex[texEmissionMatrix]);
