...
layout(location = 6)uniform vec2 lightCutOff;
...
void main(void) {
	...
	vec3 diffuse = intensity * attenuation * lightDiffuse * max(dot(norm, lightDir), 0.0) * texture(diffuseTex, fs_in.tex).rgb; 
	vec3 specular = intensity * attenuation * lightSpecular * pow(max(dot(viewDir, reflectDir), 0.0), matShininess) * texture(specularTex, fs_in.tex).rgb;  
