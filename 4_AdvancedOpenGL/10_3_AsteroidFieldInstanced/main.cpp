...
void setupProgram() {
	...
	cout<<glshaderCreate(&vertexShader, GL_VERTEX_SHADER, "shaders/drawInst.vert", DL_SHADER_CORE, 460);
	cout<<glshaderCreate(&fragmentShader, GL_FRAGMENT_SHADER, "shaders/drawInst.frag", DL_SHADER_CORE, 460);

	cout<<glprogramCreate(&programInst, "Draw Instanced", { vertexShader, fragmentShader });
...
void init(void) {
	...
	glBindBuffer(GL_ARRAY_BUFFER, glo.vbo[vboModelMat]);
...
void render(void) {
	...
	glUseProgram(programInst.programObject);
	glUniformMatrix4fv(0, 1, GL_FALSE, perspective(45.0f, winSize.w / winSize.h, 0.1f, 1000.0f));
	glUniformMatrix4fv(1, 1, GL_FALSE, lookat(glcamera.position, glcamera.front + glcamera.position, glcamera.up));
	glUniformMatrix4fv(2, 1, GL_FALSE, rotate(time, 0.0f, 1.0f, 0.0f));
	drawModel(&rock, ASTEROID_COUNT);
}
