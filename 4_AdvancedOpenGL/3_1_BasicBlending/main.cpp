...
void init(void) {
	...
	cout<<createTexture2D(&glo.tex[texGrass], "../../resources/grass.png");
...
void render(void) {
	...
	glBindVertexArray(glo.vao[vaoGrass]);
	glBindTexture(GL_TEXTURE_2D, glo.tex[texGrass]);
	for (unsigned int i = 0; i < 5; i++)
	{
		glUniformMatrix4fv(2, 1, GL_FALSE, translate(vegetation[i]));
		glDrawArrays(GL_TRIANGLES, 0, 6);
	}
}
