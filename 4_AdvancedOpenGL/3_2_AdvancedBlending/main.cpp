...
void init(void) {
	...
	glBindVertexArray(glo.vao[vaoGrass]);
	glBindBuffer(GL_ARRAY_BUFFER, glo.bo[vboGrass]);
	glBufferData(GL_ARRAY_BUFFER, sizeof(float) * len(transparentVertices), transparentVertices, GL_STATIC_DRAW);
	glEnableVertexAttribArray(0);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(vec3) + sizeof(vec2), (void*)(0));
	glEnableVertexAttribArray(1);
	glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(vec3) + sizeof(vec2), (void*)(sizeof(vec3)));
	...
	cout<<createTexture2D(&glo.tex[texWindow], "../../resources/window.png");
...
void render(void) {
	...
	{
		float dist = distance(glcamera.position, windows[i]);
	...
	glBindVertexArray(glo.vao[vaoGrass]);
	glBindTexture(GL_TEXTURE_2D, glo.tex[texWindow]);
	for (map<float, vec3>::reverse_iterator it = sorted.rbegin(); it != sorted.rend(); ++it) {
		glUniformMatrix4fv(2, 1, GL_FALSE, translate(it->second));
		glDrawArrays(GL_TRIANGLES, 0, 6);
	}
}
