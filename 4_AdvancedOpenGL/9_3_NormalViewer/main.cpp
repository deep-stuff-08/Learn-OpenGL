...
void setupProgram() {
	...
	cout<<glshaderCreate(&vertexShader, GL_VERTEX_SHADER, "shaders/normal.vert", DL_SHADER_CORE, 460);
	cout<<glshaderCreate(&geometryShader, GL_GEOMETRY_SHADER, "shaders/normal.geom", DL_SHADER_CORE, 460);
	cout<<glshaderCreate(&fragmentShader, GL_FRAGMENT_SHADER, "shaders/normal.frag", DL_SHADER_CORE, 460);

	cout<<glprogramCreate(&programNormal, "Normal", { vertexShader, geometryShader, fragmentShader });
...
void render(void) {
	...
	glUniformMatrix4fv(2, 1, GL_FALSE, rotate((float)getTime() * 40.0f, 1.0f, 1.0f, 0.0f));	
	drawModel(&model);

	glUseProgram(programNormal.programObject);
	glUniformMatrix4fv(0, 1, GL_FALSE, perspective(45.0f, winSize.w / winSize.h, 0.1f, 100.0f));
	glUniformMatrix4fv(1, 1, GL_FALSE, lookat(glcamera.position, glcamera.front + glcamera.position, glcamera.up));
	glUniformMatrix4fv(2, 1, GL_FALSE, rotate((float)getTime() * 40.0f, 1.0f, 1.0f, 0.0f));	
	drawModel(&model);
